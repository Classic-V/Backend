...
namespace Backend.Modules.Jobs
{
    public class MoneyTransportJobModule : Module<MoneyTransportJobModule>, IEventColshape
    {
        ...
        private async void Quit(ClPlayer player, string eventKey)
        {
            ...
            {
                await player.Notify("Geldtransporter", "Dein Geldtransporter wurde zerst�rt. Du erh�lst kein Geld.", NotificationType.INFO);
                job.ResetRoute();
                return;
            }
            ...
            {
                await player.Notify("Geldtransporter", "Dein Fahrzeug ist zuweit entfernt um es zur�ck zu geben!", NotificationType.ERROR);
                return;
            }
        ...
        private async void Return(ClPlayer player, string eventKey)
        {
            ...
            if (job.ReturnedAllNotes())
            {
                await player.Notify("Geldtransporter", "Du hast bereits alle Banknoten zur�ckgegeben! Gebe das Fahrzeug ab und erhalte dein Geld!", NotificationType.INFO);
                return;
            }
            ...
            await player.Notify("Geldtransporter", "Du hast alle Banknoten zur�ckgegeben! Gebe das Fahrzeug ab und erhalte dein Geld!", NotificationType.INFO);
            MarkerStreamer.RemoveMarker(job.RouteMarker.Id);
            player.Emit("Client:PlayerModule:SetWaypoint", MoneyTransportJobModel.StartPosition.X, MoneyTransportJobModel.StartPosition.Y);
        }
