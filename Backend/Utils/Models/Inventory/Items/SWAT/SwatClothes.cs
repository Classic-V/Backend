...
{
	public class SwatClothes : ItemBase
	{
		public override int Id => 30;

		public SwatClothes() : base("SWAT Einsatzkleidung", 2f, 1)
		{
		}

		public override async Task<bool> Use(ClPlayer player, int slot, int amount)
		{
			if (player.DbModel.PlayerCachedClothes != null || player.DbModel.Team < 1 || player.DbModel.Team > 2 || !player.DbModel.TeamStoragePermission || !player.DbModel.SwatDuty) return false;
